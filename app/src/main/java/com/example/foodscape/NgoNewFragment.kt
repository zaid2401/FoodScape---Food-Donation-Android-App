...
class NgoNewFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_ngo_new, container, false)

        recyclerView = view.findViewById(R.id.newdatarecyclerview)
        context?.let {
            recyclerView.layoutManager = LinearLayoutManager(it)
        }

        val confirm = view.findViewById<Button>(R.id.confirmbtn)
        ...
        confirm.setOnClickListener {
            ...
            database.collection("donation").get().addOnSuccessListener { querySnapshot ->
                for (document in querySnapshot) {
                    ...
                    database.collection("pendingdonation").add(data).addOnSuccessListener { documentReference ->
                        context?.let {
